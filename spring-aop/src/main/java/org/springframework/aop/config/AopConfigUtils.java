...
public abstract class AopConfigUtils {
	...
	private static BeanDefinition registerOrEscalateApcAsRequired(Class<?> cls, BeanDefinitionRegistry registry, Object source) {
		...
		// 检查注册表中是否已经包含了 org.springframework.aop.config.internalAutoProxyCreator 名称的Bean定义信息
		if (registry.containsBeanDefinition(AUTO_PROXY_CREATOR_BEAN_NAME)) {
			BeanDefinition apcDefinition = registry.getBeanDefinition(AUTO_PROXY_CREATOR_BEAN_NAME);
			// 如果包含了，判断是否是 AnnotationAwareAspectJAutoProxyCreator
			if (!cls.getName().equals(apcDefinition.getBeanClassName())) {
				// 如果不是 AnnotationAwareAspectJAutoProxyCreator
				// 则根据创建器的优先级，覆盖。AnnotationAwareAspectJAutoProxyCreator优先级最低
		...
		// 如果注册表中没有包含，则将 AnnotationAwareAspectJAutoProxyCreator 类的信息注册
