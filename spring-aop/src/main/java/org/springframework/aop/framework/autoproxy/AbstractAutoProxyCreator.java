...
public abstract class AbstractAutoProxyCreator extends ProxyProcessorSupport
	...
	protected Object wrapIfNecessary(Object bean, String beanName, Object cacheKey) {
		...
		// Create proxy if we have advice. 如果有定义通知，则创建代理对象
	...
	protected Object createProxy(
		...
		if (!proxyFactory.isProxyTargetClass()) {	// 返回是否直接代理目标类以及任何接口。
			if (shouldProxyTargetClass(beanClass, beanName)) {
				...
				evaluateProxyInterfaces(beanClass, proxyFactory);	// 判断是代理类还是代理接口
			}
		}

		Advisor[] advisors = buildAdvisors(beanName, specificInterceptors);
		proxyFactory.addAdvisors(advisors);				// 添加通知器
		proxyFactory.setTargetSource(targetSource);		// 设置目标对象
