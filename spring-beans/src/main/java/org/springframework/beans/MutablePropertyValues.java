...
public class MutablePropertyValues implements PropertyValues, Serializable {
	...
	public MutablePropertyValues(PropertyValues original) {
		...
		// ⾸先判断PropertyValues是否为null.
		if (original != null) {
			// 如果不为null的话,依次遍历PropertyValues的 PropertyValues.
			PropertyValue[] pvs = original.getPropertyValues();
			this.propertyValueList = new ArrayList<PropertyValue>(pvs.length);
			for (PropertyValue pv : pvs) {
				// 加⼊到MutablePropertyValues中的propertyValueList.这样 MutablePropertyValues就变相的拥有了environment中source的配置。
