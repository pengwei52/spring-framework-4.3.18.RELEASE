 ...
 */
/* 
 * 就是这个后置处理器在处理 @Autowired和@Value 注解。此外它也支持JSR-330中的@Inject注解。
 * 当我们使用 <context:annotation-config /> 或 <context:component-scan /> 时，IoC容器也会启用这个功能。
...
public class AutowiredAnnotationBeanPostProcessor extends InstantiationAwareBeanPostProcessorAdapter
	...
	public AutowiredAnnotationBeanPostProcessor() {
		// 初始化注解的类型是@Autowried,@Value以及@Inject
				...
				synchronized (this) {
					if (!this.cached) {
						if (value != null || this.required) {
							...
							registerDependentBeans(beanName, autowiredBeanNames);	// 依赖注入
		...
		protected void inject(Object bean, String beanName, PropertyValues pvs) throws Throwable {
			...
			if (arguments != null) {
				try {
					...
					method.invoke(bean, arguments);	// 反射调用
				}
