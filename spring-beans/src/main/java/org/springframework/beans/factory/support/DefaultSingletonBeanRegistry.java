 ...
 */
// 对接口SingletonBeanRegistry个函数的实现
public class DefaultSingletonBeanRegistry extends SimpleAliasRegistry implements SingletonBeanRegistry {
	...
	// 单例对象工厂的缓存:bean名称——> ObjectFactory
	...
	// 当前正在创建的bean的名称,即Bean还没有完全创建完成，还是一个半成品
	...
	protected Object getSingleton(String beanName, boolean allowEarlyReference) {
		...
		// 如果获取的对象还没有创建完成，但是正在创建（解决循环依赖的问题）
	...
	public Object getSingleton(String beanName, ObjectFactory<?> singletonFactory) {
		...
		synchronized (this.singletonObjects) {
			...
			if (singletonObject == null) {
				...
				beforeSingletonCreation(beanName);// 创建单例Bean的前处理，标记为实例正在初始化
	 ...
	 * @param beanName the name of the bean
	 */
	public boolean isSingletonCurrentlyInCreation(String beanName) {	// 单实例对象是否在创建过程中
	...
	public void destroySingletons() {	// 单例对象销毁
		if (logger.isDebugEnabled()) {
