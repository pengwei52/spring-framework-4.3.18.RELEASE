...
public abstract class TransactionAspectSupport implements BeanFactoryAware, InitializingBean {
	...
	protected Object invokeWithinTransaction(Method method, Class<?> targetClass, final InvocationCallback invocation)
		...
		if (txAttr == null || !(tm instanceof CallbackPreferringPlatformTransactionManager)) {
			...
			try {
				...
				// 这是一个around通知:调用拦截器链中的下一个拦截器。直到调用目标对象。
				retVal = invocation.proceedWithInvocation();
			}
			catch (Throwable ex) {	// 事务发生异常，回滚
				// target invocation exception
				completeTransactionAfterThrowing(txInfo, ex);
				throw ex;
			}
			finally {
				// 重置TransactionInfo ThreadLocal。在所有情况下都调用这个函数:exception或normal return!
	...
	protected void completeTransactionAfterThrowing(TransactionInfo txInfo, Throwable ex) {
		if (txInfo != null && txInfo.hasTransaction()) {
			...
			if (txInfo.transactionAttribute.rollbackOn(ex)) {
				...
				// 不回滚这个异常。如果TransactionStatus.isRollbackOnly()为真，则仍然回滚。
				try {
					txInfo.getTransactionManager().commit(txInfo.getTransactionStatus());
				}
				catch (TransactionSystemException ex2) {
	 ...
	 */
	// 用于保存事务信息。子类必须将其传递回该类上的方法，但不能看到其内部结构。
